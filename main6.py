...
def handle_message_events(event, say):
    ...
    if BOT_USER_ID in text:
        ...
        processed_sql = """
SELECT *
FROM tp_live.imported_bank_transactions
WHERE MONTH(importDate) = 10;
        ...
        if is_safe_select is False:
            ...
            try:
                output_text="abc"
                headers, data_rows = sql_utils.execute_sql_excel(processed_sql)
                file_name = sql_utils.create_excel_file(headers, data_rows)

                say(text=str(output_text), channel=channel, file=file_name) 
